...
public class RestController {
    ...
    */
    ...
    /**
     * @api {post} /v1/orders/:orderId/cancel Cancelar Orden
     * @apiName Cancela una orden
     * @apiGroup Orden
     *
     * @apiUse AuthHeader
     *
     * @apiExample {json} Body
     *   {
     *       "reason": "FAULTY_PRODUCT | DELIVERY | WRONG_PRODUCT",
     *   }
     *
     * @apiSuccessExample {json} Respuesta
     *   HTTP/1.1 200 OK
     ...
     * @apiUse Errors
     */
    public static String processCancelOrder(Request req, Response res) {
        try {
            User user = TokenService.getUser(req.headers("Authorization"));

            String orderId = req.params(":orderid");

            CancelData cancel = CancelData.fromJson(req.body());

            cancel.userId = user.id;
            cancel.orderId = orderId;

            Event event = EventService.getInstance().cancelOrder(cancel);
            BatchService.cancelOrder(event);
