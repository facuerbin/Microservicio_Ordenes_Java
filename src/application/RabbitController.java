...
public class RabbitController {
    ...
    /**
     * @api {topic} order/order-canceled Orden Cancelada
     * @apiGroup RabbitMQ POST
     * @apiDescription Envía de mensajes order-canceled desde Order con el topic "order_canceled".
     * @apiSuccessExample {json} Mensaje
     * {
     * "type": "order-canceled",
     * "message" : {
     ...
     * }
     */
    public static void sendOrderCanceled(Event event) {
        RabbitEvent eventToSend = new RabbitEvent();
        eventToSend.type = "order-canceled";
        eventToSend.exchange = "order-canceled";
        eventToSend.queue = "order";

        eventToSend.message = new OrderCanceledResponse(event.getOrderId().toHexString(), event.getCancel().getReason(), event.getCancel().getUserId());

        TopicPublisher.publish("order-canceled", "order-canceled", eventToSend);
    }

    private static class OrderCanceledResponse {
        @SerializedName("orderId")
        public String orderId;
        ...
        OrderCanceledResponse(String orderId, CancelOrderEvent.Reason reason, String userId) {
            this.orderId = orderId;
            this.reason = reason;
            this.userId = userId;
        }
